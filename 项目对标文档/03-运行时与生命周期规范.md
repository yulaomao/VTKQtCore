...
当前运行时外侧新增了一层产品启动与模块包装配层，真实主路径如下：
...
sequenceDiagram
	...
	participant Packs as ModulePackRegistry
	participant Init as ProductSoftwareInitializer
	participant Runtime as LogicRuntime

	Main->>Launcher: runProductApplication(..., ProductDefinition)
	Launcher->>Product: 读取默认 profile / 产品元数据
	Launcher->>Packs: registerModulePacks()
	Launcher->>Init: 创建 ProductSoftwareInitializer
	Init->>Runtime: initialize(...)
```

这层的职责边界如下：

- ProductDefinition 决定当前产品身份、默认 profile、窗口图标与模块集合。
- ModulePackRegistry 决定哪些业务模块在当前进程内可被注册。
- ProductSoftwareInitializer 决定如何把已声明模块挂接到 BaseSoftwareInitializer 生命周期上。
- LogicRuntime 仍只负责动作路由、模块激活、状态样本处理和通知回传，不直接参与产品选择。
