...
│  ├─ UiAction
│  ├─ LogicNotification
│  └─ ModuleInvokeRequest / ModuleInvokeResult
│
├─ Logic Layer
│  ├─ LogicRuntime
│  ├─ IModuleInvoker
...
4. 决定软件启用哪些模块、模块展示顺序、默认激活模块以及需要装配哪些可选 shell module。
...
1. 软件初始化类决定主模块、附属模块、可选 shell module、模块展示顺序和默认激活模块，并完成注册。
2. PageManager 仅负责 centerStack 中主模块的切换；附属模块和可选 shell module 可挂载到 rightWidget 或 bottomWidget 而不进入 PageManager。
...
7. BaseSoftwareInitializer 只提供 shell host、协调器和通用启动骨架，不直接实例化具体流程菜单、状态条或设备面板；这些控件必须作为模块或可选 shell module 由装配层显式挂载。

### 5.1 模块形态

框架将 UI 装配对象划分为主模块、模块附属 widget 和可选 shell module。主模块进入 centerStack，由 PageManager 切换；模块附属 widget 挂载到 rightWidget 或 bottomWidget；可选 shell module 也可挂载到这些宿主，但它们仍然是模块实现，不属于框架内建对象。

约束：

1. 模块集合由软件初始化类决定，不是全局固定集合。
2. 不同软件可以复用同一批模块实现，但模块展示顺序、initialModule、enabledModules 可以不同。
3. 主模块可带零个或多个附属 widget，也可在同一主页面内组合多个 3D 窗口。
4. 流程菜单、状态条、设备状态板、导航监控面板等壳层控件不属于框架内建对象；它们应以模块或模块附属 widget 形式存在，框架只保证 topWidget、rightWidget、bottomWidget 这些宿主稳定存在。
5. 共享启动骨架不能把某个具体模块写死在 BaseSoftwareInitializer 或 WorkspaceShell 中。
...
LogicRuntime
   ...
   └─ 按 targetModule / module / currentModule 路由到 ModuleLogicHandler
...
除 UI 到逻辑层的外部边界外，框架还允许逻辑层模块之间做受控的内部调用。

该机制用于以下场景：

1. planning 请求 datagen 创建规划预览模型。
2. navigation 请求 datagen 创建测试 TransformNode。
3. params 或 pointpick 请求其它模块执行轻量逻辑动作。

这条链路不属于 UI 边界，因此不复用 UiAction，也不通过 LogicNotification 充当请求返回协议。

统一契约：

1. ModuleInvokeRequest：sourceModule、targetModule、method、timestampMs、payload。
2. ModuleInvokeResult：ok、errorCode、message、payload。

统一入口：

1. LogicRuntime 实现 IModuleInvoker，作为模块内部调用的总转接点。
2. ModuleLogicHandler 基类持有注入的 IModuleInvoker，并对所有模块暴露受保护的 invokeModule 助手。
3. 各具体模块通过 handleModuleInvoke 处理来自其它模块的内部请求。

标准调用链：

```text
Source ModuleLogicHandler
   └─ invokeModule(targetModule, method, payload)
      └─ LogicRuntime::invokeModule(...)
         └─ ModuleLogicRegistry::getHandler(targetModule)
            └─ Target ModuleLogicHandler::handleModuleInvoke(...)
               └─ ModuleInvokeResult
```

关键约束：

1. 模块不得直接持有其它模块具体 handler 的强类型指针，不允许把 ModuleLogicRegistry 当作模块间 service locator 在业务代码中到处使用。
2. 模块内部调用默认不经过当前激活模块校验；否则跨模块协作会被误判为非法 UI 动作。
3. ModuleInvokeRequest 只承载模块间协作所需的最小参数，不应携带大体量网格、长序列采样等高频数据。
4. 内部调用的返回值只用于逻辑层协作，不直接作为 UI 刷新协议；任何会影响 UI 的结果仍由目标模块发出 LogicNotification。
5. 稳定能力优先收敛为少量 method，例如 create_node、seed_demo、query_state；不要把页面局部按钮行为原样扩散成大量随意字符串。
6. 若某类跨模块能力已经稳定，可在 ModuleInvoke 机制之上进一步抽象为类型化 capability interface；但 capability 解析仍应通过 LogicRuntime 注入，不应让模块直接构造彼此实现类。

推荐分工：

1. UiAction：UI 或服务端控制面发起的外部动作；可显式携带 targetModule 做跨模块外部路由。
2. ModuleInvoke：逻辑层模块之间的内部调用。
3. LogicNotification：逻辑层返回给 UI 的结果通知。
...
2. SceneGraph 读写、ActiveModuleState 状态判断、ModuleLogicHandler 处理均在主线程执行。
...
1. 业务动作一律异步。
2. 连接状态这类轻量信息可同步获取。
3. 任何会触发模块路由、SceneGraph 更新、Redis 通信的动作都不要阻塞 UI。
...
1. ModuleLogicHandler 之间不直接互相持有具体类型指针，统一通过基类注入的 invokeModule 助手经 LogicRuntime 转接。
2. 逻辑侧模块协同不经过 ILogicGateway，因为它不是 UI 与逻辑层边界通信，而是逻辑层内部协作。
3. 模块内部调用的请求/响应协议统一使用 ModuleInvokeRequest / ModuleInvokeResult；不得复用 LogicNotification 承担同步返回值语义。
4. 内部调用失败时，调用方模块可根据 ModuleInvokeResult 决定是否向当前模块 UI 发出错误 LogicNotification，但目标模块不应反向直接操作调用方 UI。
5. 跨模块共享的大体量场景真相仍然放在 SceneGraph，ModuleInvoke 只用于触发动作、查询轻量结果和能力协作。
...
2. 壳层显示结果（模块切换、连接状态）由 ApplicationCoordinator 分发；模块细节显示结果由 ModuleCoordinator 分发。
3. 当前激活模块状态保留在 ActiveModuleState，模块临时状态保留在 ModuleLogicHandler。
...
1. 接收 UiAction。
2. 维护当前激活模块状态，并在 request_switch_module 时完成模块切换。
3. 根据 payload.targetModule、action.module 或当前激活模块，把动作或 Redis 消息路由给对应 ModuleLogicHandler。
4. 维护跨模块共享的 SceneGraph。
5. 通过通信层与 Redis 交互。
6. 在需要时通过注入的 IRedisCommandAccess 主动查询或写入 Redis；这条路径使用独立永久连接，不与控制面 command 连接、订阅连接或轮询连接复用。
7. 实现 IModuleInvoker，并作为模块逻辑内部调用的统一转接点。
...
7. 在需要时通过注入的 IRedisCommandAccess 主动补拉模块状态、执行模块级 60Hz 查询/写入或回写模块确认结果，但不得直接持有 RedisGateway。
8. 通过基类提供的 invokeModule 助手调用其它模块的受控能力，或通过 handleModuleInvoke 暴露本模块愿意提供给其它模块的能力。
...
2. 决定全局模块切换。
3. 直接操作别的模块 QWidget。
4. 直接 new 其它模块处理器或依赖其它模块的具体实现类。

---

## 10. ActiveModuleState 的职责

ActiveModuleState 在当前版本中只保留“当前激活模块状态”的职责；它不承担全局流程裁判或模块准入控制。

它负责：

1. 记录 currentModule。
2. 记录 initialModule，供启动和重同步快照使用。
3. 在模块切换发生后，为壳层提供统一的当前模块真相。

它不负责：

1. 维护 enterableModules。
2. 解释 next_step、prev_step 的全局顺序语义。
3. 判定任意 UiAction 当前是否允许执行。
4. 约束 CustomAction 只能发往当前模块。

约束：

1. 模块展示顺序属于软件装配信息，可由 BaseSoftwareInitializer 或具体软件 profile 决定，但该顺序只用于 UI 展示和默认激活模块选择，不参与动作合法性判断。
2. 当 Redis 或逻辑结果表明模块变化时，ActiveModuleState 负责记录 currentModule 真相。
3. ApplicationCoordinator 只根据 currentModule 真相切页。
4. 具体进入该模块后的数据刷新，由对应模块处理器、显示管理层和 ModuleCoordinator 分工接管；其中 3D 场景由显示管理层刷新，普通模块界面由 ModuleCoordinator 刷新。
5. 对外部动作的业务合法性校验应下沉到目标模块或软件自定义策略，而不是继续由框架级状态机统一裁决。
...
1. RedisGateway：负责 Redis command 连接与 pub/sub 连接的生命周期、基础 key-value 读写、publish 和重连。
2. RedisLogicCommandAccess：为 LogicRuntime 和 ModuleLogicHandler 提供独立永久 command 连接，只负责 read、write 和 publish，不承担订阅。
3. MessageRouter：负责控制面消息分类、通道映射、基础字段校验和客户端/服务端消息路由。
4. SourceBase、SubscriptionSource、PollingSource、PollingTask 以及轮询线程内的 RedisPollingWorker：负责运行订阅分发源和遍历轮询源，并输出 StateSample；轮询 GET 复用独立持久连接，不为每次读取临时创建线程和 TCP 连接。
5. CommunicationHub：负责整合控制面消息和数据面状态输入，再交给 LogicRuntime 或对应 ModuleLogicHandler；它不承担逻辑层主动 Redis 命令连接本体，只负责通信编排。
...
4. 再由该初始化类创建 MainWindow、PageManager、ActiveModuleState、模块处理器和各模块 UI。
...
2. 服务端下发的模块同步或控制命令必须先进入 LogicRuntime，再由 ActiveModuleState 和 ModuleLogicHandler 决定是否落地，不允许 Redis 数据直接驱动 QWidget。
...
5. 高频轮询不得走“每次 GET 临时起线程、临时建 TCP 连接”的实现；轮询线程必须复用独立持久 Redis 连接，避免 60Hz 场景下出现线程风暴和连接抖动。
...
2. PollingSource 运行在独立轮询线程或独立调度器中，不与 pub/sub 监听线程混合；轮询线程内的 RedisPollingWorker 复用持久 command 连接执行 GET。
...
### 19.5.1 LogicRuntime 与模块逻辑的主动 Redis 访问

除订阅分发和遍历轮询外，框架还保留一条主动 Redis 访问路径，供 LogicRuntime 和 ModuleLogicHandler 通过独立永久连接主动查询和写入。

推荐用途：

1. 启动后或模块激活时补拉最小必要配置。
2. 用户确认、参数提交或轻量状态变更后的 key-value 回写。
3. 通信恢复后的最小状态补拉，而不是等待下一次被动推送。
4. 模块逻辑或 LogicRuntime 内部明确建模的高频主动查询/写入场景，例如每秒 60 次的 query/write，但这类访问也必须走同一条独立永久连接，而不是临时建连。

约束：

1. 该能力通过注入的 IRedisCommandAccess 暴露给 LogicRuntime 和 ModuleLogicHandler，不允许逻辑层直接持有 RedisGateway。
2. UI 仍然不能直接访问 Redis；UI 只能通过 ILogicGateway 向逻辑层发动作。
3. 这条主动访问路径使用独立永久 command 连接，不与 RedisGateway 的控制面 command 连接、pub/sub 连接和 RedisPollingWorker 的轮询连接复用。
4. 当 LogicRuntime 或 ModuleLogicHandler 存在每秒 60 次级别的主动 query/write 需求时，应统一走这条连接，而不是在业务逻辑里反复 new context 或反复起线程。
5. 主动写入可使用 writeValue、writeJsonValue 或 publishMessage、publishJsonMessage；应优先写入明确的协议 key 或通道，避免模块各自发明隐式命名。
6. 该连接不承担订阅监听；需要订阅时仍旧必须走 RedisGateway + SubscriptionSource。
...
3. ModuleLogicHandler 负责把高频状态样本转换为模块内部状态、SceneGraph 更新或轻量通知；服务端下发的控制消息由 LogicRuntime 先解析目标模块，再按显式 targetModule、module 或当前激活模块路由。
...
7. LogicRuntime 或 ModuleLogicHandler 的主动 Redis 查询结果若需要进入稳态刷新链路，也应先落到模块状态或 SceneGraph，再由既有通知链和显示管理层继续传播；不能直接绕过逻辑层去驱动 QWidget 或 vtkActor。
...
2. 连接恢复后自动恢复订阅，并重新启动轮询任务；轮询线程内的持久 polling connection 和 LogicRuntime/ModuleLogicHandler 的专用 command 连接也按需重建。
3. 客户端自动发送 resync_request。
4. 服务端若在重连后重新下发模块切换或自定义控制指令，客户端仍需先经 LogicRuntime 解析目标模块与动作类型。
5. LogicRuntime 先恢复当前模块状态，并可通过专用 IRedisCommandAccess 连接主动补拉全局最小必要状态。
6. 对应 ModuleLogicHandler 再恢复模块最小必要状态和实时缓存，必要时通过专用 IRedisCommandAccess 连接补拉模块级 key-value 或执行恢复性写回。
...
1. 选点侧的壳层导航入口或模块页面发出 UiAction。
2. 选点模块的 ModuleCoordinator 调用 ILogicGateway。
3. LogicRuntime 解析 request_switch_module 的 targetModule，并将当前激活模块切到规划模块。
...
│  ├─ redis/
│  │  ├─ RedisGateway.h/.cpp
│  │  ├─ RedisLogicCommandAccess.h/.cpp
│  │  └─ RedisPollingWorker.h/.cpp
│  └─ routing/
│     └─ MessageRouter.h/.cpp
├─ contracts/
│  ├─ UiAction.h
...
4. 再把 ActiveModuleState 收缩为当前激活模块状态容器，并定义显式 targetModule 路由规则；模块展示顺序仅保留在软件装配层。
...
8. Redis 数据获取同时支持订阅分发、遍历轮询和逻辑层受约束的主动查询/写入；高频来源仍需先统一转换为标准化状态样本，再进入 ModuleLogicHandler，不直接暴露给 UI。
9. 高频轮询必须复用独立持久 Redis 连接，不能把每次 GET 实现为临时线程和临时 TCP 连接；否则 60Hz 场景下会出现线程风暴与连接抖动。
10. 高频 transform、pose 与设备状态采用 latest-wins、节流和阈值过滤策略，不按逐条完整通知回流 UI。
11. 数据面派生出的 synthetic 通知必须与服务端基线通知分离，并在缓存回灌路径中显式去环，避免 synthetic scene_nodes_updated 被再次当作基线输入处理。
12. 壳层导航只消费软件装配层给出的模块展示顺序；direct switch 与 CustomAction 的可达性不再由框架级状态机统一约束，而改由显式 targetModule 路由和目标模块自定义校验控制。
13. 实时 3D 渲染必须遵守“逻辑层更新 SceneGraph、显示管理层增量消费”的原则；在高频流场景下优先复用 actor，而不是把 scene_nodes_updated 实现成每帧全量重建渲染对象。
14. 壳层中的 globalOverlayLayer 与 globalToolHost 默认按“可承载交互组件”设计；不要把其根层永久配置为鼠标穿透，否则确认层、全局面板和工具窗会在结构上失去交互能力。
